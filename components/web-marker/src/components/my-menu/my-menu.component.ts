...
export class MyMarkElement extends LitElement {
  ...
  render() {
    ...
    <!-- Add / remove mark -->
        <button tabindex="1" class="btn ${this.markId ? 'active' : ''}" @click=${async (e: MouseEvent) => await this.toggleMark(e)}>
    ...
    <!-- Go to Homepage => UserIcon -->
      <button tabindex="3" class="btn user" @click=${() => navigateExternal('https://marius96.uber.space/')}>
